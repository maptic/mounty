    ...
    var body: some View {
        HStack(spacing: 12) {
            Image(systemName: "server.rack")
                ...
                .help(isMounted ? "Mounted at: \(currentPath)" : "Server: \(volume.serverAddress)")
            ...
            HStack(spacing: 8) {
                Button { manager.toggleAutomount(volume.id) } label: {
                    Image(systemName: volume.isAutomountEnabled ? "bolt.fill" : "bolt")
                        .font(.system(size: 14))
                        .foregroundColor(volume.isAutomountEnabled ? .orange : .secondary.opacity(0.3))
                }
                .buttonStyle(.plain).help(volume.isAutomountEnabled ? "Disable Automount" : "Enable Automount")
                
                if isMounted {
                    Button { manager.openInTerminal(volume) } label: {
                ...
                Button { if isMounted { manager.unmount(volume) } else { manager.mount(volume) } } label: {
        ...
        .onTapGesture(count: 2) { if isMounted { manager.openInFinder(volume) } }
        .contextMenu {
            Button(role: .destructive) { manager.removeVolume(volume.id) } label: { Text("Remove Volume") }
        }
    }
}
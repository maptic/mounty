...
class VolumeManager: ObservableObject {
    ...
    private func setupPipelines() {
        ...
        eventMonitor.interfacesChanged
            .receive(on: RunLoop.main)
            .sink { [weak self] in
                logger.info("Interface topology changed. Retrying connections.")
                Task {
                    await self?.refreshState()
        ...
        // Use a less frequent heartbeat now that events are primary trigger
    ...
    private func runAutomount() async {
        ...
        for volume in volumes where volume.isAutomountEnabled {
            if mountPaths[volume.id] == nil && !busyVolumes.contains(volume.id) {
                ...
                let isReachable = await ReachabilityService.isServerReachable(address: volume.serverAddress)
                if isReachable && mountPaths[volume.id] == nil {
                    guard let url = URL(string: volume.serverAddress) else { continue }
                    logger.info("Automounting volume: \(volume.name, privacy: .public)")
    ...
    func refreshState() async {
        ...
        let newPaths = await Task.detached {
            return await VolumeManager.detectMounts(volumes: currentVolumes, isNetworkUp: networkAvailable)
        }.value
    ...
    nonisolated private static func detectMounts(volumes: [Volume], isNetworkUp: Bool) async -> [UUID: String] {
        ...
        await withTaskGroup(of: (UUID, String?).self) { group in
            for volume in volumes {
                group.addTask {
                    if let path = SystemMountService.findMountPath(for: volume, in: systemMounts) {
                        if await ReachabilityService.isServerReachable(address: volume.serverAddress) {
    ...
    func mount(_ volume: Volume) {
        ...
        Task {
            if let path = await MountService.mount(url: url) {
                self.mountPaths[volume.id] = path
                logger.info("Manually mounted volume: \(volume.name, privacy: .public)")
            } else {
    ...
    func openInFinder(_ volume: Volume) {
        if let path = mountPaths[volume.id] { MountService.openInFinder(path: path) }
    }
    ...
    // MARK: - Persistence & Import/Export
    
    func addVolume(_ volume: Volume) {
        volumes.append(volume); storage.saveVolumes(volumes)
        Task { await refreshState() }
    }
    
    func removeVolume(_ id: UUID) {
        volumes.removeAll { $0.id == id }; storage.saveVolumes(volumes)
        Task { await refreshState() }
    }
    
    func clearAllVolumes() {
        volumes.removeAll(); storage.saveVolumes(volumes)
        Task { await refreshState() }
    }
    ...
    func importVolumes(from data: Data) throws {
        ...
        for volume in importedVolumes {
            if !volumes.contains(where: { $0.serverAddress == volume.serverAddress }) {
    ...
    func toggleAutomount(_ id: UUID) {
        if let idx = volumes.firstIndex(where: { $0.id == id }) {
            volumes[idx].isAutomountEnabled.toggle(); storage.saveVolumes(volumes)
    ...
    private func disableAutomount(for volume: Volume) {
        if let idx = volumes.firstIndex(where: { $0.id == volume.id }) {
            volumes[idx].isAutomountEnabled = false; storage.saveVolumes(volumes)
        }
    }
    ...
    func setPreferredTerminal(_ bundleID: String) {
        preferredTerminal = bundleID; storage.saveTerminalBundleID(bundleID)
    }
}
